...
import { generateCommercialURL } from "@/helpers/generateCommercialURL";
    ...
    <>
      ...
      >
        <IndividualButtonWithLink
          link={`/commercial${generateCommercialURL({
    ...
    <div className="inline-block" ref={dropdownRef}>
      ...
      {isOpen && (
        ...
        >
          {options.map((option) => {
            if (name === "type") {
              const url = `/commercial${generateCommercialURL({
                cityVal: city,
                houseTypeVal: option,
    ...
    <div className="inline-flex sm:mr-4 flex-wrap gap-y-2">
      {options.map((option, index) => {
        return (
          ...
          >
            <Link
              href={`/commercial${generateCommercialURL({
                saleLeaseVal: option,
                cityVal: city,
                houseTypeVal: type,
              })}`}
